  ...
  const exerciseData = {
    ...
    inputs: [
      { 
        ...
        label: 'אורך הניצב', 
        type: 'number', 
        placeholder: 'הכנס תשובה בס"מ',
        answerKey: 'answer'
      }
    ],
    answers: { answer: 6.43 },
    tolerance: 0.1,
    validator: CommonValidators.trigonometry.basic,
    hints: [
      'השתמש בפונקציית הסינוס',
      'sin(40°) = ניצב מול / יתר',
      'ניצב מול = יתר × sin(40°)'
    ],
    solution: {
      steps: [
        'נסמן את הניצב שמול הזווית ב-a',
        'נשתמש בפונקציית הסינוס: sin(40°) = a/10',
        'a = 10 × sin(40°) ≈ 10 × 0.6428 ≈ 6.43',
        'התשובה: 6.43 ס״מ'
      ]
    },
    customContent: <TriangleDiagram angle={40} adjacentSide={10 * Math.cos(40 * Math.PI / 180)} showValues={false} />
  };
  const quizQuestions = [
    {
  ...
  return (
    ...
    >
      ...
      <section className="mb-12">
        <h2 className="text-2xl font-semibold text-purple-600 mb-6 border-b-2 border-purple-200 pb-2">
          מתרגלים ✍️
        </h2>        <InteractiveExercise {...exerciseData} />
      </section>
