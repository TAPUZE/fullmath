    ...
    if (questions && Array.isArray(questions) && questions.length > 0) {
      const shuffledQuestions = questions.map(question => {
        if (question.options && Array.isArray(question.options)) {
          // Create array of options with their original values
          const optionsWithValues = question.options.map((option, index) => ({
            label: typeof option === 'string' ? option : option.label,
            value: typeof option === 'string' ? option : (option.value || option.label)
          }));
          
          // Shuffle the options
          const shuffledOptions = [...optionsWithValues].sort(() => Math.random() - 0.5);
